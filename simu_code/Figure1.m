...
%     x = gen_dep(n,p,1, corl) * S;
    ...
    ghat1 = yhat1 - dhat1*rfsss1(ii,1);  % get the estimation for g(x)
    gy1 = y(osamp,:)-ghat1;
    rfgsss1(ii,1) = d(osamp,:)\gy1; % get the estimation for theta from ordinary regression --> least squares estimation
...
mean_D = mean(abs(rfsss(:,1) - alpha)./rfsss(:,2) > 1.96);
disp(mean_D);   % t-statistics
disp('RF - Nonorthogonal (Y) + sample splitting'); 
mean_Y = mean(abs(rfgsss(:,1) - alpha)./rfgsss(:,2) > 1.96);
disp(mean_Y);
disp('RF - Orthogonal + sample splitting');
mean_O = mean(abs(rfsds(:,1) - alpha)./rfsds(:,2) > 1.96);
disp(mean_O);
...
ti = sprintf('FIG%0.2f.fig',corl);
savefig(f,ti);
skew_sds = skewness(pds);
kurto_sds = kurtosis(pds);
mean_sds = mean(pds);
skew_gss = skewness(pss);
kurto_gss = kurtosis(pss);
mean_gss = mean(pss);
T = table(skew_sds,kurto_sds,mean_sds,skew_gss,kurto_gss,mean_gss, mean_D, mean_Y, mean_O);


function std = iid_se(X, error, X_inv)
    n = size(X, 1);
    p = size(X, 2);
    
    var = X_inv * (1/(n-p)) * (error' * error);
    std = var^0.5;
end

function x = gen_dep(T,d,sigma,cor)
    echo off;
    x = zeros(T,d);
